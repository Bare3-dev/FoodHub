...
/**
 * Low Priority Job: Sync menu items from POS system
 * 
 * This job handles menu synchronization from POS systems
 * with retry logic and exponential backoff for reliability.
 */
class MenuSyncFromPOSJob implements ShouldQueue
{
     ...
     */
    public $timeout = 300; // 5 minutes max (menu sync can take longer)
    ...
    public function __construct(
        ...
        private readonly ?string $priority = 'low'
    ) {
    ...
    {
        try {
            Log::info('Starting POS menu sync', [
            ...
            // Attempt to sync menu from POS
            $result = $posService->syncMenuItems($this->restaurant, $this->posType);

            if ($result['success']) {
                Log::info('POS menu sync successful', [
                    'restaurant_id' => $this->restaurant->id,
                    'pos_type' => $this->posType,
            ...
            Log::error('POS menu sync failed', [
                'restaurant_id' => $this->restaurant->id,
                'pos_type' => $this->posType,
                'attempt' => $this->attempts(),
                'error' => $e->getMessage()
    ...
    {
        Log::critical('POS menu sync job failed permanently', [
            'restaurant_id' => $this->restaurant->id,
            'pos_type' => $this->posType,
    ...
    {
        $cacheKey = "pos.menu.sync.success.{$this->posType}.{$this->restaurant->id}";
        Cache::put($cacheKey, [
            'synced_at' => now(),
            'pos_type' => $this->posType,
            'restaurant_id' => $this->restaurant->id
        ], 7200); // Cache for 2 hours (menu changes less frequently)
    }
    ...
    {
        $cacheKey = "pos.menu.sync.failed.{$this->posType}.{$this->restaurant->id}";
        Cache::put($cacheKey, [
            'failed_at' => now(),
            'error' => $error,
            'pos_type' => $this->posType,
