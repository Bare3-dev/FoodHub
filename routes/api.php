...
/*
|--------------------------------------------------------------------------
| API Routes with Comprehensive Security Stack & Versioning
|--------------------------------------------------------------------------
|
| Routes are organized by API version with a complete security stack:
...
|
| Version Management:
| - v1: Current stable version (all existing endpoints)
| - v2: Future breaking changes (when needed)
| - Automatic deprecation warnings and migration guidance
| - 6-12 month backward compatibility for critical endpoints
|
*/

// Global API version info route
Route::middleware([\App\Http\Middleware\ApiVersionMiddleware::class])->get('/version', function() {
    $currentVersion = \App\Models\ApiVersion::getCurrentVersion();
    $deprecatedVersions = \App\Models\ApiVersion::getDeprecatedVersions();
    
    return response()->json([
        'current_version' => $currentVersion?->version ?? 'v1',
        'status' => $currentVersion?->status ?? 'active',
        'deprecated_versions' => $deprecatedVersions->pluck('version'),
...
// ============================================================================
Route::middleware([\App\Http\Middleware\ApiVersionMiddleware::class])->prefix('v1')->group(function () {
    ...
    Route::group(['middleware' => ['api.cors:public', 'advanced.rate.limit:general']], function () {
        ...
        // Device token management - accessible by all users for push notifications
        Route::post('/device-tokens/register', [DeviceTokenController::class, 'register']);
        Route::post('/device-tokens/remove', [DeviceTokenController::class, 'remove']);
        Route::get('/device-tokens/user', [DeviceTokenController::class, 'getUserTokens']);
        Route::post('/device-tokens/validate', [DeviceTokenController::class, 'validateToken']);
        Route::post('/device-tokens/test', [DeviceTokenController::class, 'sendTestNotification']);
    });
    ...
    Route::group(['middleware' => ['auth:sanctum', \App\Http\Middleware\UserStatusMiddleware::class, 'api.cors:private', 'advanced.rate.limit:general']], function () {
        // User info
    ...
    Route::group(['middleware' => ['auth:sanctum', \App\Http\Middleware\UserStatusMiddleware::class, 'https.security']], function () {
        ...
        Route::group(['middleware' => 'role.permission:SUPER_ADMIN|RESTAURANT_OWNER|BRANCH_MANAGER'], function () {
            ...
            // File upload management (restricted)
        ...
        Route::group(['middleware' => 'role.permission:SUPER_ADMIN|DELIVERY_MANAGER'], function () {
            ...
            Route::prefix('delivery')->group(function () {
                ...
                // Real-time location tracking - REMOVED DUPLICATE ROUTE
...
Route::get('/migration/check', function () {
    return response()->json([
        'current_version' => 'v1',
        'status' => 'stable',
        'sunset_date' => null,
...
Route::get('/migration/guide', function () {
    return response()->json([
        'message' => 'No migration required for v1',
        'current_version' => 'v1',
        'status' => 'stable'
    ]);
});
...
Route::get('/analytics', [App\Http\Controllers\Api\ApiVersionAnalyticsController::class, 'index']);
Route::get('/analytics/{version}', [App\Http\Controllers\Api\ApiVersionAnalyticsController::class, 'show']);
Route::get('/analytics/migration/progress', [App\Http\Controllers\Api\ApiVersionAnalyticsController::class, 'migrationProgress']);
Route::get('/analytics/monitoring/realtime', [App\Http\Controllers\Api\ApiVersionAnalyticsController::class, 'realTimeMonitoring']);

// Queue monitoring endpoints (admin only)
Route::middleware(['auth:sanctum', 'role.permission:SUPER_ADMIN|RESTAURANT_OWNER|RESTAURANT_MANAGER'])->group(function () {
    Route::get('/queue/monitoring', [App\Http\Controllers\Api\QueueMonitoringController::class, 'index']);
    Route::get('/queue/monitoring/realtime', [App\Http\Controllers\Api\QueueMonitoringController::class, 'realTime']);
    Route::get('/queue/monitoring/pos-sync-status', [App\Http\Controllers\Api\QueueMonitoringController::class, 'getPOSSyncStatus']);
    
    // Job management endpoints (owner/admin only)
    Route::middleware(['role.permission:SUPER_ADMIN|RESTAURANT_OWNER'])->group(function () {
        Route::post('/queue/jobs/retry', [App\Http\Controllers\Api\QueueMonitoringController::class, 'retryJob']);
        Route::delete('/queue/jobs/failed', [App\Http\Controllers\Api\QueueMonitoringController::class, 'clearFailedJobs']);
    });
});
});
...
// ============================================================================
Route::middleware([\App\Http\Middleware\ApiVersionMiddleware::class])->prefix('v2')->group(function () {
    // This section will contain future breaking changes
    // Currently empty - will be populated when v2 is needed
    
    // Example of how v2 endpoints would be structured:
    // Route::get('/restaurants', [RestaurantController::class, 'indexV2']);
    // Route::post('/orders', [OrderController::class, 'storeV2']);
...
Route::get('/migration/check', function () {
    return response()->json([
        ...
        'migration_guide' => config('app.url') . '/api/docs/migration/v1-to-v2'
    ]);
});
...
// ============================================================================
// WILDCARD ROUTE FOR INVALID API VERSIONS
// ============================================================================
// This route must be placed LAST to catch any invalid version patterns
// It allows the ApiVersionMiddleware to handle invalid versions properly
Route::middleware([\App\Http\Middleware\ApiVersionMiddleware::class])->group(function () {
    Route::any('/{version}/{path?}', function ($version, $path = null) {
        // This route will never be reached if the middleware works correctly
        // It's just a fallback to ensure the middleware runs for invalid versions
        abort(404);
    })->where('version', 'v\d+')->where('path', '.*');
});

// ============================================================================
// LEGACY ROUTES (DEPRECATED - WILL BE REMOVED IN FUTURE VERSIONS)
// ============================================================================
// These routes maintain backward compatibility but will be deprecated
